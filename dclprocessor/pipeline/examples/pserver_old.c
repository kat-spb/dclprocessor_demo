...
int main(int argc, char *argv[]) {
    ...
    if (argc != 2) { 
        ...
        fprintf(stdout, "\t %s minimal\t-- pipeline with one final thread, do nothing\n", argv[0]);
        fprintf(stdout, "\t %s demo\t-- pipeline with 2 modules (first and middle) and final thread, do nothing\n", argv[0]);
        fprintf(stdout, "\t %s data/pipeline_example.json\t-- pipeline from config\n", argv[0]);
        fflush(stdout);
        return 0;
    }
    ...
    while (1) {
        if (argc == 2) {
            create_pipeline_description(argv[1], &pdesc);
